...
module.exports = async (msg) => {
  ...
  const vegasembed = new MessageEmbed().setColor(client.vegasRenkler[Math.floor(Math.random() * client.vegasRenkler.length)]).setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true, size: 2048 }));
  args = args.splice(1);
  ...
  if (cmd) {
    cmd.run({client: client, msg: msg, args: args, embed: vegasembed, prefix: prefix, uye: uye, author: author, Database: Database, guild: guild, cfg: cfg, MessageEmbed: MessageEmbed, Discord: Discord});
  }
};
